...
use super::linear_issues::{get_session_linear_identifiers, LinearIssueContext};
...
use super::types::{
    ...
    WorktreeDeleteErrorEvent, WorktreeDeletedEvent, WorktreeDeletingEvent,
};
use crate::chat::types::{LabelData, Session};
...
pub async fn create_worktree(
    ...
    let auto_pull_base_branch = if pr_context.is_none() {
        crate::load_preferences(app.clone())
            .await
            .map(|preferences| preferences.auto_pull_base_branch)
            .unwrap_or(false)
    } else {
        false
    };
    let service = crate::backend_runtime::project_service(&app)?;
    let events: Arc<dyn jean_core::EventSink> =
        Arc::new(crate::backend_runtime::DesktopEventSink::new(app.clone()));
    let (pending, task) = service
        .prepare_worktree(
            jean_core::WorktreeCreationInput {
                project_id,
                base_branch,
        ...
        .map_err(|error| error.to_string())?;
    allow_project_in_asset_scope(&app, task.project_path());
    allow_project_in_asset_scope(&app, task.worktrees_root());
    let pending = serde_json::from_value(pending)
        .map_err(|error| format!("Failed to decode pending worktree: {error}"))?;
    thread::spawn(move || service.run_worktree_task(task, events.as_ref()));
    Ok(pending)
}
...
pub async fn fork_session_to_worktree(
    ...
    let response = crate::backend_runtime::project_service(&app)?
        ...
        .map_err(|error| error.to_string())?;
    let response: ForkSessionToWorktreeResponse =
        serde_json::from_value(response).map_err(|error| error.to_string())?;
    allow_project_in_asset_scope(&app, &response.worktree.path);
    Ok(response)
}
...
pub async fn checkout_pr(
    ...
    let context = crate::backend_runtime::context(&app)?;
    match service
        .prepare_checkout_pr(&project_id, pr_number, context.events.as_ref())
        .map_err(|error| error.to_string())?
    {
        jean_core::CheckoutPrPreparation::Restored(worktree) => {
            serde_json::from_value(worktree).map_err(|error| error.to_string())
        }
        jean_core::CheckoutPrPreparation::Create { pending, task } => {
            allow_project_in_asset_scope(&app, task.project_path());
            allow_project_in_asset_scope(&app, task.worktrees_root());
            let worktree = serde_json::from_value(pending).map_err(|error| error.to_string())?;
            let events = context.events.clone();
            thread::spawn(move || service.run_worktree_task(*task, events.as_ref()));
            Ok(worktree)
        }
